...
class HBSMAT:
    ...
    def matvec(self,v):
        ...
        n0 = 0
        qhat = torch.zeros((0,vperm.shape[1]),dtype=v.dtype)
        #########################
        #       UPWARD PASS
        #########################
        ...
        uperm = torch.zeros((vperm.shape[0],vperm.shape[1]),dtype=v.dtype)
...
hbs_mat = HBSMAT(t,Om,Psi,Y,Z,rk)


v=torch.randn((A.shape[1],1),dtype=torch.float64)
start = time.time()
u = hbs_mat.matvec(v)
stop = time.time()
tMatVec = stop-start
...
print("err = ",tla.norm(Av-u)/tla.norm(Av))
print("tMatVec = ",tMatVec)
print("torig = ",torig)

print("err = ",tla.norm(ATv-uT)/tla.norm(ATv))
