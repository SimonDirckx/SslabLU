...
for indp in range(len(pvec)):
    ...
    tic = time.time()
    S_rk_list,rhs_list,Ntot,nc = OMS_rk.construct_Stot_helper(bc,assembler,dbg=1)
    t_constr_rk = time.time()-tic

    tic = time.time()
    S_lu_list,rhs_list,Ntot,nc = OMS_lu.construct_Stot_helper(bc,dbg=1)
