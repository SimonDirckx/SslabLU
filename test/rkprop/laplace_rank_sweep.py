...
def bc(p):
    pp = np.array(p)                 # accepts torch tensors (hpsalt) or numpy
    rr = np.sqrt((pp[:, 0] - src[0])**2 + (pp[:, 1] - src[1])**2)
    if kh == 0:
        return np.log(rr)
    else:
        return j0(kh * rr)
...
# so Laplace is c11 = c22 = 1; coefficients are torch callables f(xx)->tensor
if kh == 0:
    pdo_lap = pdo.PDO_2d(c11=pdo.const(1.), c22=pdo.const(1.))
else:
    pdo_lap = pdo.PDO_2d(c11=pdo.const(1.), c22=pdo.const(1.), c=pdo.const(-kh*kh))
...
for ind in range(len(rkvec)):
    ...
    err[ind]  = np.linalg.norm(uhat - u_true) / np.linalg.norm(u_true)
    tvec[ind] = tsolve
...
print("domain / discretization : unit square, hpsalt (torch HPS)")
...
print("%6s | %12s | %12s | %10s | %8s" %
      ("rk", "rel err", "rel res", "solve [s]", "compr."))
print("----------------------------------------------------------------")
for ind in range(len(rkvec)):
...
try:
    ...
    axD[0, 1].semilogy(ys[order], abserr[sl][order] + tiny, '.-')
    ...
    figD.suptitle('Test 1: Laplace on unit square (hpsalt), rank sweep', fontsize=12)
    ...
    axF[0].set_title('exact   u = log|x - x0|'); figF.colorbar(s0, ax=axF[0], shrink=0.85)
    ...
    s2 = axF[2].scatter(XY[:, 0], XY[:, 1], c=np.log10(abserr + tiny), s=10, cmap='magma')
