...
def compute_QRW(Dtot,Vtot,Nb,n,k):
    NN = (n-k)*Nb
...
def compute_ULV(Umats,Dmats,Vmats,nl,k0,n,k,Nbvec,SHBS):
    ...
    for i in range(len(Dmats)):
        ...
        if i==0:
            Rprime = Dmats[0]
            Q,W,R_11,R_12,R_22,NN = compute_QRW(Rprime,Vmats[0],Nbvec[0],nl,k0)
        else:
            Rprime = U@Dmats[i]+Rprime[:,NN:]
            Q,W,R_11,R_12,R_22,NN = compute_QRW(Rprime[NNtot+NN:,:],Vmats[i],Nbvec[i],n,k)
