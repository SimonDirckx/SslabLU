...
class partition:
    ...
    # natural 3D generalization of the 2D construction:
    #    _____________
    #   |      |      |
    #   |      |      |
    #   |      |      |
    ...
    # Boundaries: ga,gb,g (of a, b and total resp.)t
     ...
     def classify(self,p):
          ...
          y=p[1]
          z=p[2]
          if np.abs(y)<1e-15 or np.abs(y-self.Ly)<1e-15 or np.abs(z)<1e-15 or np.abs(z-self.Lz)<1e-15 :
               return 0
          elif np.abs(x)<1e-15:
               return 1
...
class discretization:
    ...
    def __init__(self,ordx:int,ordy:int,ordz:int,discType:str):
    ...
    def discretize(self,H,Ly,Lz):
        ...
        if self.discType == 'cheb':    
            ...
            zpts = -spectral.cheb_pts(self.ordz)
            xpts = self.H*(xpts+1.)
            ypts = .5*self.Ly*(ypts+1.)
            zpts = .5*self.Lz*(zpts+1.)
            self.xpts=xpts
            self.ypts=ypts
            self.zpts=zpts
            XYZ=np.zeros(shape=((self.ordx+1)*(self.ordy+1)*(self.ordz+1),3))
            
            for i in range(self.ordx+1):
                x = xpts[i]
                for j in range(self.ordy+1):
                    y = ypts[j]
                    for l in range(self.ordz+1):
                        z = zpts[l]
                        XYZ[l+j*(self.ordz+1)+i*(self.ordy+1)*(self.ordz+1),:] = [x,y,z]
            self.XYZ=XYZ
...
class dSlab:
    ...
    def __init__(self,disc:discretization,part:partition,localToGlobal,H,Ly,Lz): # future: add prblm:problem
        
        #self.prblm = prblm
        self.disc   =   disc
        disc.discretize(H,Ly,Lz)
        self.part   =   part
    ...
    def l2g(self,i):
         ...
         y=self.dofs[i,1]
         z=self.dofs[i,2]
         return self.localToGlobal(x,y,z)
    ...
    def computeL(self):
        ...
        Ey  = self.disc.get_Ey()
        Ez  = self.disc.get_Ez()
        L = np.kron(np.kron(Ex,Ey),Dzz)+np.kron(np.kron(Ex,Dyy),Ez)+np.kron(np.kron(Dxx,Ey),Ez)
        self.L=L
